...
public class DbConnectorTest {
	/**
	 * Testing the default constructor, does a simple select statement
	 */
	@Ignore
	@Test
	...
	/**
	 * Testing the non-default constructor does a simple select statement
	 */
	@Ignore
	@Test
	...
	/**
	 * This method tests the addition of a new row to the database, after the
	 * row is added the row will be deleted
	 */
	@Ignore
	@Test
	public void testInsertRow() {
		...
		String qry1 = "SELECT COUNT(*) FROM change_history a WHERE a.revision =" + revision + " AND a.pdb_id='" + pdb_id
				+ "'";
		...
		try {
			...
			if (rowCount == 0) {
				// add the new entry
				String insertQry = "INSERT INTO change_history VALUES('" + revision + "','" + pdb_id + "','"
						+ currentDate + "')";
			...
			// else if you get a result then add it with a new revision
				...
				String qry = "SELECT revision FROM change_history WHERE pdb_id='" + pdb_id
						+ "' AND revision=(SELECT MAX(revision) FROM change_history)";
				...
				String qry3 = "INSERT INTO change_history VALUES('" + new_revision + "','" + pdb_id + "','"
						+ currentDate + "')";
	...
	/**
	 * This method tests the querying functionality. It should only return the
	 * latest (unique) revision given the query's revision
	 */
	@Ignore
	@Test
